...
class FrequencyPath(nn.Module):
    ...
    def forward(self, x):
        ...
        # Dynamically generate frequency filter for current spatial dimensions
        h, w = F_x.shape[-2], F_x.shape[-1]
        weight = self.dynamic_freq_filter(h, w, x.device)  # shape: (1, in_channels, h, w)
        F_x = weight * F_x
        # Work with the real part for splitting (simplification)
        F_low, F_high = self.split_frequency(F_x.real)
        F_prime = self.phi(F_low, F_high)
        # Inverse FFT reconstruction (using only the real part)
...
###############################################
# Module I: DNA-Inspired Dual-Domain Block
###############################################
...
if __name__ == '__main__':
    ...
    # Test with various spatial dimensions, including odd sizes.
