...
def main():
    ...
    warmup_scheduler, cosine_scheduler = define_scheduler(optimizer)
    ...
    if config.resume:
        ...
        if checkpoint["warmup_scheduler"] is not None:
            warmup_scheduler.load_state_dict(checkpoint["warmup_scheduler"])
        if checkpoint["cosine_scheduler"] is not None:
    ...
    for epoch in range(config.start_epoch, config.epochs):
        ...
        if epoch < warmup_epochs:
            warmup_scheduler.step()  # Use warmup scheduler
        else:
            cosine_scheduler.step()  # Use cosine annealing scheduler
        ...
        torch.save({"epoch": epoch + 1,
                    ...
                    "cosine_schedular": cosine_scheduler.state_dict()},
...
def define_optimizer(model) -> optim.Adam:
    ...
    optimizer = optim.AdamW(model.parameters(), lr=3e-4, betas=(0.9, 0.99), weight_decay=1e-4)
...
def warmup_lambda(epoch):
    return min(1.0, (epoch + 1) / warmup_epochs)  # Scale LR from 0 to 1
...
def define_scheduler(optimizer):
    ...
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=2, eta_min=5e-5)

    # Create warmup scheduler
    warmup_scheduler = LambdaLR(optimizer, lr_lambda=warmup_lambda)

    # Create cosine scheduler after warmup
