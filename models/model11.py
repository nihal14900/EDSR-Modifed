...
class CrossAttentionBlock(nn.Module):
    """ Full block combining parallel conv pathways and cross-attention for Super-Resolution """
    def __init__(self, in_channels, out_channels, min_window=6, num_heads=4):
        super().__init__()
        # Two pathways: One with standard 3x3 Conv, another with Dilated 3x3 Conv
        ...
        # Cross-attention module with adaptive windowing
    ...
    def forward(self, x):
        ...
        x1 = self.conv3x3(x)          # Standard 3x3
        x2 = self.conv3x3_dilated(x)  # Dilated 3x3 (dilation=2)

        # Cross-attention modulation
        x1_modulated, x2_modulated = self.cross_attention(x1, x2)

        # Concatenate and fuse
        out = torch.cat([x1_modulated, x2_modulated], dim=1)  # Concatenate along channel dim
        out = self.relu(self.bn(self.fusion_conv(out)))  # Fuse features
