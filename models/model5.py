...
class ChannelAttention(nn.Module):
    """
    Simple Squeeze-and-Excitation-based channel attention.
    ...
    def __init__(self, channels, reduction_ratio=16):
        ...
        # Hidden dimension for the "bottleneck"
    ...
    def forward(self, x):
        ...
        # 1) Squeeze: Global Average Pool (spatial dims)
        # shape => (B, C)
        ...
        # 3) Reshape attention back to (B, C, 1, 1) and scale
...
class DoubleTripleDNABlock_Separate_CA(nn.Module):
    ...
    + Channel Attention after final triple bond.
    """
    def __init__(self, channels, kernel_size=3, 
                 dbl_num_interactions=2,   # typically 2
                 trp_num_interactions=3,   # typically 3
                 ca_reduction=16):         # reduction ratio in channel attention
        ...
        self.U12_trp = nn.ModuleList([
            nn.Conv2d(channels, channels, kernel_size, padding=kernel_size // 2)
            for _ in range(trp_num_interactions)
        ])
        self.U21_trp = nn.ModuleList([
            nn.Conv2d(channels, channels, kernel_size, padding=kernel_size // 2)
            for _ in range(trp_num_interactions)
        ])
        
        # -- Channel Attention (applied after the triple bond)
        self.ca = ChannelAttention(channels, reduction_ratio=ca_reduction)
    ...
    def forward(self, x):
        ...
        #    Two cross-interaction steps (separate weights)
        ...
        #    Three cross-interaction steps (separate weights)
        ...
        # 5) Channel Attention after final triple bond
        M_trp_ca = self.ca(M_trp)
...
if __name__ == "__main__":
    ...
    # Create the Double-Triple DNA block with separate weights and channel attention
