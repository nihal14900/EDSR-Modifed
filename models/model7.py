...
# ---------------------------
# Cross-Attention Block with Separate Convs for Initial and Mid Stages
# ---------------------------
class CrossAttentionBlock(nn.Module):
    def __init__(self, in_channels, reduction=16, kernel_size=7):
        ...
        Implements the block with:
         ...
         2. Double bond branch with dual conv paths and separate cross-channel attention layers.
         3. Two separate intermediate convolutions (mid conv) whose outputs are merged.
         4. Triple bond branch with dual conv paths and separate cross-spatial attention layers.
        ...
        # ---------------------------
        # 2. Double Bond Branch (Cross-Channel Attention)
        # ---------------------------
        ...
        # ---------------------------
        # 4. Triple Bond Branch (Cross-Spatial Attention)
        # ---------------------------
        # Two parallel convolutions.
    ...
    def forward(self, x):
        ...
        z1_dbl = self.relu(self.conv_dbl1(z0))
        z2_dbl = self.relu(self.conv_dbl2(z0))
        ...
        # ---------------------------
        z1_mid = self.relu(self.conv_mid1(M_dbl))
        z2_mid = self.relu(self.conv_mid2(M_dbl))
        # Merge mid features.
        ...
        # ---------------------------
        # a. Parallel feature extraction.
        z1_trp = self.relu(self.conv_trp1(z1))
        z2_trp = self.relu(self.conv_trp2(z1))
