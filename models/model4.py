...
class EDSR(nn.Module):
    def __init__(self, upscale_factor: int) -> None:
        ...
        trunk = []
        for _ in range(16):
            trunk.append(DoubleTripleDNABlock_Separate(
                channels=64, 
                kernel_size=3,
...
class DoubleTripleDNABlock_Separate(nn.Module):
    """
    A block that uses:
      1) Initial convolution
      2) Double bond (2 cross-interactions) with separate weights
      3) Intermediate convolution
      4) Triple bond (3 cross-interactions) with separate weights
      5) Residual skip connection
    """
    def __init__(self, channels, kernel_size=3, 
                 dbl_num_interactions=2,  # Typically 2
                 trp_num_interactions=3): # Typically 3
        super(DoubleTripleDNABlock_Separate, self).__init__()
        ...
        #    We'll store them in nn.ModuleList
...
if __name__ == "__main__":
    ...
    # Create the Double-Triple DNA block (Separate Weights)
    block_sep = DoubleTripleDNABlock_Separate(
        channels=64, 
        kernel_size=3,
        dbl_num_interactions=2,  # double bond steps
        trp_num_interactions=3   # triple bond steps
